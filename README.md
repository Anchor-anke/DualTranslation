...
它不是代码编辑器，也不会自动读取你的仓库或替你执行命令。它专注于一件事：减少人与 Coding Agent 之间因表达不完整、技术术语和验证口径不同而产生的误解。

## 主要功能

- **快速模式与协商模式**
  - 快速模式会使用低风险、可逆的假设直接生成任务。
  - 协商模式会在关键条件缺失时提出 1–3 个真正影响实现的问题。
...
2. 新增一个模型服务配置。
3. 填写模型信息并保存。
4. 回到“转换”，选择“写给 Agent”或“看懂 Agent”。
...
- 应用不会读取代码仓库、自动运行命令或自动操作 Coding Agent。
...
用户输入
   ...
   ├─ 敏感信息扫描与用户确认
   │
   ├─ 写给 Agent ──> 规范化任务 JSON ──> Codex / Cursor / 通用提示词
   │
...
- 优先使用“快速模式”。对于官方 DeepSeek 接口，该模式会关闭模型默认思考过程。
